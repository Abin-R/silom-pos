...
from bravepos.models import (
    DEFAULT_STOCK_OUT_REASONS, Branch, BranchSession, Settings, Staff,
    StockDocument, StockMovement, StockOutReason,
)
...
class StockOutReasonSeedTests(TestCase):
    ...
    def test_reasons_carry_thai_labels(self):
        ...
        waste = StockOutReason.objects.get(branch=branch, name="Waste / Expired")
        self.assertEqual(waste.name_th, "ของเสีย / หมดอายุ")
...
class StockOutReasonApiTests(TestCase):
    ...
    def test_list_returns_only_this_branch_active_reasons(self):
        ...
        hidden = StockOutReason.objects.get(branch=self.branch, name="Other")
        hidden.active = False
        hidden.save()
    ...
    def test_stock_out_stores_the_reason(self):
        ...
        self.assertEqual(res.json()["reason"], "Waste / Expired")
        self.assertEqual(StockDocument.objects.get().reason, "Waste / Expired")
    ...
    def test_the_reason_reaches_the_movement_ledger(self):
        ...
        self._stock_out()
        self.assertEqual(StockMovement.objects.get(type="out").note, "Waste / Expired")
    ...
    def test_renaming_a_reason_does_not_rewrite_saved_documents(self):
        ...
        self._stock_out()
        row = StockOutReason.objects.get(branch=self.branch, name="Waste / Expired")
        row.name = "Spoilage"
        row.save()
        self.assertEqual(StockDocument.objects.get().reason, "Waste / Expired")
    ...
    def test_deleting_a_reason_does_not_touch_saved_documents(self):
        self._stock_out()
        StockOutReason.objects.filter(branch=self.branch, name="Waste / Expired").delete()
        self.assertEqual(StockDocument.objects.get().reason, "Waste / Expired")
    ...
    def test_reason_is_ignored_on_a_stock_in(self):
        ...
        res = self.client.post(
            "/api/stock-documents",
            {
                "type": "in", "reason": "Waste / Expired", "vendor": "Supplier",
                "items": [{
...
class StockMovementExportTests(TestCase):
    ...
    def test_a_stock_out_row_carries_qty_and_reason(self):
        self._stock_out("Waste / Expired", 3)
        rows = self._export()
        row = rows[8]
